...
class InquiryManufacturing(models.Model):
    _name = 'inquiry.manufacturing.line'
    _inherit = 'inquiry.tools.supplies'
    _description = 'inquiry manufacturing line'

    product_id = fields.Many2one('product.product', string="Product", change_default=True, ondelete='restrict',
                                 index='btree_not_null',
                                 domain="[('detailed_type', '=', 'product'), ('sale_ok', '=', True), ('purchase_ok', '=', False)]")
    ...
    state = fields.Selection(
        ...
        string=_("Status"), copy=False, related='inquiry_id.state')
    ...
    def _calc_prices(self):
        for rec in self:
            ...
            if bqr_obj:
                for line in bqr_obj.bqr_manufacturing_product_ids:
                    if rec.product_id.id == line.product_id.id:
                        cost += line.total_manu_finish_cost
    ...
    def _compute_grouped_lines(self):
        grouped_lines = defaultdict(lambda: self.env['inquiry.manufacturing.line'])
